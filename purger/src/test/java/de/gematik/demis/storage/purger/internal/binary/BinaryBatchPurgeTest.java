...
class BinaryBatchPurgeTest {

  @Mock private BatchesConfiguration batchesConfiguration;
  @Mock private BinaryBatchPurgeRepository repository;
  @Mock private Batch batch;
  @InjectMocks private BinaryBatchPurge binaryBatchPurge;

  @Test
  void givenNormalStateWhenNameThenBinary() {
    assertEquals("binary", binaryBatchPurge.name());
  }
  ...
  void givenBatchSizeFiveWhenPurgeBatchThenPurgeFiveAtRepositoryAndComplete() {
    ...
    // given
    final List<UUID> purgedBinaries =
        IntStream.range(0, 5).mapToObj(i -> randomUUID()).collect(Collectors.toList());
    when(repository.purgeExpiredRecords()).thenReturn(purgedBinaries);

    // when
    binaryBatchPurge.purgeBatch(batch);

    // then
    verify(repository, times(1)).purgeExpiredRecords();
    verify(batch, times(1)).complete(purgedBinaries);
  }
  ...
  void givenExceptionWhenPurgeExpiredRecordsThenCompleteExceptionally() {
    ...
    // when
    CompletableFuture<Void> future = binaryBatchPurge.run();
